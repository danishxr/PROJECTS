...
f<-ggplot(chemicals_present,aes(chemicals_present$CHEMICAL,
chemicals_present$FREQ_REPORTED))
...
labs(title="Trend of Specific Chemical Composition in different states of India",x= "States", y="Number_Of_Cases",fill="CHEMICALS")+scale_fill_brewer( type = "qua", palette = "Dark2", direction = 1)+
...
#as.numeric(plea$Quality.Parameter)
...
ggmap(indiamap)+geom_point(data=plea,aes(x=longitude,y=latitude,colour=Quality.Parameter,size=2),alpha=.5,na.rm=TRUE)+
  scale_colour_brewer(type = "seq", palette = "Spectral", direction = 1)+labs(title="Heat Map-Major chemical chemicals_present",fill="CHEMICALS")+
theme(plot.title = element_text(face="bold.italic"),legend.box.background = element_rect(),legend.box.margin = margin(6, 6, 6, 6))
